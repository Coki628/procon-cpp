 ...
 * ・要素を余分に1つ番兵として持ったら、区間なしによる0を考慮しないで操作できた。
 */
...
Node compare(Node a, Node b) {
    ...
    res.sm = a.sm + b.sm;
    // max(左全部+右前半, 左前半のみ)
    res.prefmx = max({a.sm+b.prefmx, a.prefmx});
    // max(左後半+右全部, 右後半のみ)
    res.sufmx = max({a.sufmx+b.sm, b.sufmx});
    // max(左最大, 右最大, 左後半+右前半)
    res.mx = max({a.mx, b.mx, a.sufmx+b.prefmx});
    return res;
}
...
int main() {
    ...
    cin >> N >> Q;
    // 「区間なしにより0」がOKなので、番兵的に1つ余分な要素を作っておく
    SegTree<Node> seg(N+1, compare, {0, 0, 0, 0});
    ll x;
    rep(i, 0, N) {
        cin >> x;
        seg.set(i, {x, x, x, x});
