 ...
 * ・更新順とか調整したら、使い回しできた。てかLi Chao Treeは1つあればOKだった。
 * 　これでメモリは大幅改善。まあ結局TLEなんだけど。。
 * ・直線の追加をちょっと減らしてみた。AC増えたけどまだまだTLE。。
 */
...
int main() {
    ...
    DynamicLiChaoTree<ll> lct(0, 10000000000, INF);
    lct.add_line(0, 0);
    rep(i, 0, N) {
        rep(j, i, N+1) {
            dp[(i+1)&1][j] = -lct.query(T[j]);
            if (dp[(i+1)&1][j] < X+dp[i&1][j]) {
                lct.add_segment(T[j], T[j]+X, -1, -(-T[j]+dp[i&1][j]));
                lct.add_segment(T[j]+X, 10000000000, 0, -(X+dp[i&1][j]));
            }
            dp[i&1][j] = 0;
        }
        print(dp[(i+1)&1][N]);
