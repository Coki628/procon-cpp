 ...
 * ・1点更新、区間minとminな要素数取得
 * ・早速来た、ちょっと応用編。自力AC嬉しい。
 * ・2要素がマージされる時に、どういう更新をすればminな要素数を保持できるか考える。
 */
...
Node compare(Node a, Node b) {
    if (a.val < b.val) {
        ...
        // 同値ならここのマージでは両方が最小となるので要素数を足し合わせる
...
int main() {
    ...
    cin >> N >> Q;
    SegTree<Node> seg(N, compare, {INF, 0});
    ll x;
    rep(i, 0, N) {
        cin >> x;
        seg.set(i, {x, 1});
