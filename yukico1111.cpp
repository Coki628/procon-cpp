 ...
 * ・自力AC！
 * ・グラフ、3次元DP
 * ・見た目はグラフっぽい。実際、辺は張るんだけど、最短経路とかではない。
 * ・状態を持って、遷移を張られた辺だけにすれば遷移量はならしO(1)みたいになるので、
 * 　計算量300^3のDPになって通る。AC0.18秒。pypyでも通るかは分からんがこれなら多分いけそう。
 */
...
int main() {
    ...
    // dp[i][j][k] := i個目まで決めて、最後のコードがjで、複雑度がkの時の通り数
    auto dp = list3d<ll>(N+1, L, K+1, 0);
    rep(u, 0, L) {
        dp[1][u][0] = 1;
    }
    rep(i, 1, N) {
