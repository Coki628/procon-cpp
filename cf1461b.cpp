 ...
 * ・自力AC！
 * ・グリッド、累積和
 * ・普通にやると500^3はかかりそうだからC++でやろう、となる。
 * ・実際に書いてみるとそのままだと500^4になりそうだったので累積和で横を前計算。
 * 　含まれる*の数 == 区間長 になってればOK、みたいな判定にした。
 */
    ...
    rep(i, 0, H) {
        rep(j, 0, W) {
            if (grid[i][j] == '*') {
                ans++;
                ll h = i, wl = j, wr = j;
                while (h+1 < H and wl-1 >= 0 and wr+1 < W) {
                    h++;
                    wl--;
                    wr++;
                    if (acc[h][wr+1]-acc[h][wl] != wr-wl+1) {
                        break;
                    }
                    ans++;
                }
