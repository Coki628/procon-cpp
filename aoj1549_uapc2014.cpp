/**
 * 参考：https://miti-7.hatenablog.com/entry/2019/02/01/152131
 * 　　　https://ei1333.github.io/library/structure/wavelet/wavelet-matrix.cpp
 * ・ライブラリ整備
 * ・Wavelet Matrix
 * ・区間L,Rの範囲にある値で、S,Tの間にどのへんにあるかとか、何個あるかとか、何番目の数とかを
 * 　高速に答えられる。ソートしてない列に対してこれがlogでできるってすごいねー。
 */
...
int main() {
    ...
    rep(i, 0, Q) {
        ...
        r++;
        ans = INF;
        val = wm.next_value(l, r, d);
        if (val != -1) chmin(ans, val-d);
        val = wm.prev_value(l, r, d);
        if (val != -1) chmin(ans, d-val);
        print(ans);
    }
    return 0;
}
