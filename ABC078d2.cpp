 ...
 * ・蟻本演習4-2-1
 * ・ゲームDP、ミニマックス法
 * ・ちょっと手間取った。最初状態N^2*遷移NでN^3書いちゃってTLE。。
 * ・よくよく考えると、手持ち札を持っておくのは片方で大丈夫。
 * 　今の手番が最後まで取るのを毎回確認すれば、それと比較するための相手の手持ち札だけ残せばいい。
 */
...
int main() {
    ...
    auto memo = list2d(2, N, -1);
    auto rec = [&](auto&& f, bool turn, ll i) -> ll {
        if (memo[turn][i] != -1) {
            return memo[turn][i];
        }

        ll res = abs(A[N-1] - A[i]);
        if (turn == 0) {
            rep(j, i+1, N-1) {
                chmax(res, f(f, !turn, j));
            }
            memo[turn][i] = res;
            return res;
        } else {
            rep(j, i+1, N-1) {
                chmin(res, f(f, !turn, j));
            }
            memo[turn][i] = res;
            return res;
        }
