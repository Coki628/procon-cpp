/**
 * 参考：https://codeforces.com/blog/entry/53170
 * ・ライブラリ整備
 * ・HL分解、部分木クエリ
 * ・HLDは適切に構築していれば部分木クエリにも対応できる。
 * 　具体的には、vの部分木なら[in[v],out[v])の区間を取ればいい。
 */
...
int main() {
    ...
    BIT<ll> bit(N);
    rep(u, 0, N) {
        hld.add(u, u, [&](ll a, ll b) { bit.add(a, A[u]); });
    }
    ...
    rep(i, 0, Q) {
        ...
        if (op == 0) {
            cin >> x >> y;
            hld.add(x, x, [&](ll a, ll b) { bit.add(a, y); });
        } else {
            cin >> x;
            ll ans = bit.query(hld.in[x], hld.out[x]);
            print(ans);
        }
    }
    return 0;
